...
<div class="actions">
	...
	<ul>
		<li><?php echo $this->Html->link(sprintf(__('Edit %s', true), __('City', true)), array('action' => 'edit', $city['City']['id'])); ?> </li>
		<li><?php echo $this->Html->link(sprintf(__('Delete %s', true), __('City', true)), array('action' => 'delete', $city['City']['id']), null, sprintf(__('Are you sure you want to delete # %s?', true), $city['City']['id'])); ?> </li>
...
<div class="related">
	...
	<?php if (!empty($city['CustomerHome'])):?>
	...
	<?php
		$i = 0;
		foreach ($city['CustomerHome'] as $customerHome):
			$class = null;
			if ($i++ % 2 == 0) {
				$class = ' class="altrow"';
			}
